...
export default function Register({setRegister, setLogin}) {
    ...
    const submit = async (data) => {
        ...
        if (checkPasswordMatched && data.termsConditions ) {
            let response = await axios.post("http://localhost:3000/api/register", { username, email, password});
  ...
  return (
        ...
        <form onClick={(e) => e.stopPropagation()} onSubmit={handleSubmit(submit)} className="mx-auto mt-10 px-6 py-3 flex flex-col bg-white rounded-[10px] w-min h-min ">
