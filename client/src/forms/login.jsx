...
export default function login({ setRegister, setLogin, setForgotPassword }) {
    ...
    const submit = async (data) => {
        ...
        if (!response.data.success) {
            if (response.data.message === "user not found") {
                ...
                console.log("login.jsx rejected :", response.data)
  ...
  return (
        <form onClick={(e) => e.stopPropagation() } onSubmit={handleSubmit(submit)} className="rounded-[5px] flex flex-col px-5 py-4 max-w-fit self-center bg-white mx-auto">
            ...
            <div className="flex mt-2 items-center justify-between w-full">
                <label className="flex flex-row mt-2 text-[13px] items-center  cursor-pointer">
            <input type="checkbox" className="mr-1" {...register("rememberMe")} />Remember me
            </label>
            <small onClick={() => {setForgotPassword(true); setLogin(false)}} className="text-[darkorange] h-fit duration-200 cursor-pointer text-[15px hover:border-b hover:border-[darkorange]">forgot password?</small>
            </div>
